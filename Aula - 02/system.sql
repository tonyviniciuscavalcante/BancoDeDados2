...
CREATE SEQUENCE teste_seq INCREMENT BY 10 START WITH 10 MAXVALUE 9999 CYCLE NOCACHE;

INSERT INTO Teste (numero, descricao) VALUES (teste_seq.NEXTVAL, 'Joana');
INSERT INTO Teste (numero, descricao) VALUES (teste_seq.NEXTVAL, 'Tony');

SELECT * FROM Teste;
SELECT teste_seq.CURRVAL from dual;

ALTER SEQUENCE teste_seq INCREMENT BY 20 MAXVALUE 99999 NOCACHE CYCLE;
...
SELECT nome, SUBSTR(nome, 1, 5) 
FROM atleta;

SELECT nome, SUBSTR(nome, -8, 6) 
FROM clube;

SELECT nome, SUBSTR(nome, -8) 
FROM clube;

SELECT nome, LENGTH(nome) AS TAM 
FROM atleta;

SELECT nome, salario, ROUND(salario, 0), TRUNC(salario, 1)
FROM atleta;

SELECT TRIM(nome) 
FROM atleta;

SELECT LTRIM(nome), RTRIM(nome)
FROM atleta;

SELECT REPLACE (nome, 'e', 'i') 
FROM clube;
...
SELECT nome, NVL(TO_CHAR(id_clube), 'Não possui')
FROM atleta;
...
SELECT nome, salario,
    CASE WHEN salario > 50000 THEN 0.3
         ...
         ELSE 0
    END AS Imposto,
    CASE WHEN salario > 50000 THEN (salario + salario*0.3)
         WHEN salario > 25000 THEN (salario + salario*0.2)
         WHEN salario > 5000 THEN (salario + salario*0.1)
         ELSE 0
    END AS SalarioComImposto
FROM atleta;

SELECT nome, sexo, DECODE(sexo, 'M', 'Masculino', 'F', 'Feminino')
FROM atleta;

SELECT TO_CHAR(sysdate, 'yyyy-mm-dd month HH24:MI:SS AM')
FROM dual;

SELECT TO_CHAR(sysdate, 'yyyy-mm-dd month ddspth HH24:MI:SS AM')
FROM dual;

SELECT nome, TO_CHAR(datanasc, 'DD Month YYYY') AS Data_Nasc
FROM atleta;
...
SELECT id_clube, sexo, ROUND(AVG(salario), 2)
FROM atleta
...
SELECT id_clube, COUNT(id_clube) AS QtdeCentroTreinamento
FROM centro_treinamento
GROUP BY id_clube
ORDER BY id_clube;

SELECT id_clube, COUNT(id_clube)
FROM centro_treinamento
GROUP BY id_clube
HAVING COUNT(id_clube) = 1;

/*----------------------------------------------------------------------------------------------------*/
/*Exercícios – Uso de Funções*/
/* 1. Liste o nome (em maiúsculo), a data de nascimento, o bônus por
idade (calculado), o endereço e o Ɵpo de logradouro (calculado)
dos atletas. O bônus por idade funciona assim: se o empregado
nasceu anteriormente a 1990, receberá 5000, se nasceu
anteriormente a 2000 receberá 3000, senão não receberá nada. O
Ɵpo de logradouro é calculado com base nas úlƟmas 6 letras do
logradouro: se for “Avenue”, deve mostrar “Avenida”, se for
“Street” deve mostrar “Rua”, senão deve mostrar “Outro”. Use case
e decode.*/


/* 2. Liste o nome, o endereço (subsƟtuindo “Avenue” por “Av.” e “Street” por “St.”
na exibição), o id do clube (sendo que é preciso exibir “Não possui” quando não
possuir clube) e o salário formatado para exibir com uma casa obrigatória antes
da vírgula e duas casas decimais obrigatórias). */
...
/*3. Encontre a quanƟdade de atletas que praƟca cada modalidade esporƟva;*/


/*4. Para cada modalidade esporƟva praƟcada, liste o maior e o menor tempo
de experiência. Ordene os resultados por id de modalidade.*/


/*5. Liste o id do campeonato e a quanƟdade de atletas que parƟciparam
deles, exibindo somente os campeonatos com mais de 3 parƟcipantes.
Ordene os resultados por quanƟdade de parƟcipantes
decrescentemente;*/


/*6. Liste a média de colocação dos atletas parƟcipantes de campeonatos,
...
/*1. Crie uma sequence que será usada como PK da tabela TESTE. A sequence deve
começar com o valor 100 e ter um valor máximo 999. O incremento a cada valor deve
ser 23. Não uƟlize valor de cache. Denomine a sequence como TESTE_NRO_SEQ.*/


/*2. Insira 3 registros na tabela TESTE, uƟlizando a sequence criada nos comandos
INSERTS.*/
...
/*4. Insira mais 2 registros na tabela TESTE, uƟlizando a sequence criada nos comandos
INSERTS.*/
/* 5. Como você faria para reiniciar a sequence a parƟr do valor 1?*/


/*6 No script de Esporte, uƟlizado em aula, pode-se observar que os dados
inseridos para os ids nas tabelas Presidente, Clube, Atleta e Modalidade são
sequenciais e poderiam ser inseridos automaƟcamente por meio do uso de
sequences*/


/*7 Modifique o script para que os ids dessas tabelas sejam inseridos por meio de
sequences.*/
