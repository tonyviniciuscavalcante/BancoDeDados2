...
CREATE OR REPLACE PACKAGE Esporte_Pkg AS
    FUNCTION Get_Atletas_By_Clube(p_id_clube IN NUMBER)
        RETURN SYS_REFCURSOR;

    FUNCTION Count_Modalidades_By_Atleta(p_id_atleta IN NUMBER)
        RETURN NUMBER;
...
CREATE OR REPLACE PACKAGE BODY Esporte_Pkg AS
    -- Function para obter os atletas de um clube
    ...
    -- Function para contar o número de modalidades que um atleta pratica
...
-- Exemplo de Uso
-- Chamando a função Get_Atletas_By_Clube da package Esporte_Pkg:
DECLARE
...
BEGIN
    ...
    LOOP
        ...
        DBMS_OUTPUT.PUT_LINE('ID Atleta: ' || v_id_atleta || ', Nome: ' || v_nome_atleta || ', Salário: ' ||
                             v_salario || ', Data de Nascimento: ' || v_datanasc);
    END LOOP;
    -- Fecha o cursor
    CLOSE v_atletas;
END;

-- Chamando a função Count_Modalidades_By_Atleta da package:
DECLARE
...
BEGIN
    ...
    -- Exibe o resultado
    DBMS_OUTPUT.PUT_LINE('O atleta pratica ' || v_qtd_modalidades ||
                         ' modalidades.');
END;

-- Chamando a procedure Add_Atleta da package Esporte_Pkg:
BEGIN
...
-- LTER INDEX nome_índice REBUILD;

-- Para eliminar um índice, utilizamos o comando:
...
-- 7. Liste o ID do clube e a quantidade de centros de treinamento
...
-- Produto.
-- - Uma função que retorne a quantidade de produtos cadastrados na
-- tabela.
-- - Teste a package montando um programa em PL/SQL que utilize o
...
-- - Teste a package montando um programa em PL/SQL que utilize as
