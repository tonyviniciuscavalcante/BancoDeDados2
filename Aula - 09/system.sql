...
ALTER TABLE atleta
    ADD date_ult_alt DATE;
ALTER TABLE atleta
    ADD user_ult_alt VARCHAR(20);
...
(
    ...
    constraint AUDIT_ATL_SAL_ID_FK FOREIGN KEY (id_atleta) REFERENCES atleta (id)
);
...
ALTER TABLE clube
    ADD qtde_atleta NUMBER DEFAULT 0;

UPDATE clube c
SET c.qtde_atleta = (SELECT COUNT(*)
                     FROM atleta a
...
BEGIN
    IF INSERTING THEN
        UPDATE clube SET qtde_atleta = qtde_atleta + 1 WHERE id = :new.id_clube;
    ELSIF DELETING THEN
        UPDATE clube SET qtde_atleta = qtde_atleta - 1 WHERE id = :old.id_clube;
    END IF;
...
INSERT INTO atleta (id, nome, cpf, salario, datanasc, id_clube)
VALUES (121, 'Myke Tyson', '888-22-333', 60000, to_date('01-04-1975', 'dd-mm-yyyy'), 5);

INSERT INTO atleta (id, nome, cpf, salario, datanasc, id_clube)
VALUES (122, 'Xuxa', '999-11-4321', 10000, to_date('14-01-1979', 'dd-mm-yyyy'), 5);
...
SELECT user,
       to_char(data_alt, 'dd/mm/yyyy HH:mi:ss') as
           data_hora,
       operacao
FROM MODALIDADE_LOG;
...
-- Crie as tabelas acima, definindo tipos e constraints adequadas. Coloque o valor default
...
-- filme - Lançamento (1 dia) ou Normal (2 dias). Desenvolva um gatilho para gerar
...
(
    ...
    classificacao VARCHAR(20)  NOT NULL CHECK (classificacao IN ('Lançamento', 'Normal')),
    qtde_locacoes INT DEFAULT 0
);
...
(
    ...
    data_hora_emp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    data_hora_dev TIMESTAMP,
    FOREIGN KEY (cod_cli) REFERENCES Cliente (cod),
    FOREIGN KEY (cod_filme) REFERENCES Filme (cod)
);

-- Trigger para atualizar qtde_locacoes do Cliente
CREATE OR REPLACE TRIGGER trg_update_qtde_locacoes_cliente
    AFTER INSERT
    ON Aluguel
    FOR EACH ROW
BEGIN
    UPDATE Cliente
    SET qtde_locacoes = qtde_locacoes + 1
    WHERE cod = :NEW.cod_cli;
END;

-- Trigger para atualizar qtde_locacoes do Filme
CREATE OR REPLACE TRIGGER trg_update_qtde_locacoes_filme
    AFTER INSERT
    ON Aluguel
    FOR EACH ROW
BEGIN
...
CREATE OR REPLACE TRIGGER trg_set_data_hora_dev
    BEFORE INSERT
    ON Aluguel
    FOR EACH ROW
DECLARE
    v_classificacao Filme.classificacao%TYPE;
BEGIN
    -- Buscar classificação do filme
    ...
    IF v_classificacao = 'Lançamento' THEN
        :NEW.data_hora_dev := :NEW.data_hora_emp + INTERVAL '1' DAY;
    ELSE
        :NEW.data_hora_dev := :NEW.data_hora_emp + INTERVAL '2' DAY;
    END IF;
END;
...
VALUES (10, 'Filme A', 'Lançamento');
INSERT INTO Filme (cod, titulo, classificacao)
VALUES (20, 'Filme B', 'Normal');
...
-- Movimentacao (nro_conta,data_mov, tipo, valor)

-- Crie as tabelas acima, definindo tipos e constraints adequadas. Faça
-- uma check que permita somente ‘D’ ou ‘C’ para o tipo de
...
-- Teste os gatilhos por meio de inserções de dados na tabela
...
CREATE OR REPLACE TRIGGER trg_update_saldo
    AFTER INSERT
    ON Movimentacao
    FOR EACH ROW
BEGIN
    IF :NEW.tipo = 'C' THEN
        -- Crédito: adicionar valor
        UPDATE Conta
        SET saldo = saldo + :NEW.valor
        WHERE nro_conta = :NEW.nro_conta;
    ELSIF :NEW.tipo = 'D' THEN
        -- Débito: subtrair valor
        UPDATE Conta
        SET saldo = saldo - :NEW.valor
        WHERE nro_conta = :NEW.nro_conta;
    END IF;
...
INSERT INTO Conta (nro_conta, nome_cliente, saldo)
VALUES (1, 'Ana', 1000.00);
INSERT INTO Conta (nro_conta, nome_cliente, saldo)
VALUES (2, 'Bruno', 500.00);

INSERT INTO Movimentacao (nro_conta, tipo, valor)
VALUES (1, 'C', 200.00);
INSERT INTO Movimentacao (nro_conta, tipo, valor)
VALUES (2, 'D', 100.00);
INSERT INTO Movimentacao (nro_conta, tipo, valor)
