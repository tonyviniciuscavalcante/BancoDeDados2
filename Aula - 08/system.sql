...
ALTER USER tony QUOTA UNLIMITED ON USERS;
GRANT CREATE SESSION, CREATE TABLE, CREATE SEQUENCE, CREATE VIEW, CREATE PROCEDURE TO tony;
...
BEGIN
    ...
    -- se existir (v_aux>0) entao atualiza as informacoes
    IF v_aux > 0 THEN
        UPDATE atleta SET endereco = p_end, id_clube = p_clube
        WHERE id = p_id;
        DBMS_OUTPUT.PUT_LINE('Endereco e clube do atleta atualizados com sucesso!');
        COMMIT;
    END IF;
    EXCEPTION
        WHEN OTHERS THEN
...
EXEC PR_ALTERA_ATLETA_END_CLUBE (20, 'Rua do IFSP, 325', 14);
-- OU
CALL PR_ALTERA_ATLETA_END_CLUBE (20, 'Rua do IFSP, 325', 14);
SELECT * FROM atleta WHERE id = 20;

CALL PR_ALTERA_ATLETA_END_CLUBE (p_end => 'Rua do IFSP Novo, 547', p_clube => 13, p_id => 20);
SELECT * FROM atleta WHERE id = 20;

CALL PR_ALTERA_ATLETA_END_CLUBE (20, p_clube => 12, p_end => 'Av. Novo Prédio IFSP, 567');
SELECT * FROM atleta WHERE id = 20;

DROP PROCEDURE PR_ALTERA_ATLETA_END_CLUBE;


-- Exemplo de uso de parâmetro modo OUT
CREATE OR REPLACE PROCEDURE PR_SAL_CLUBE (
    p_clube IN atleta.id_clube%TYPE,
    p_media_sal OUT NUMBER) IS
BEGIN
    -- retorna a media salarial dos atletas do clube informado
    SELECT avg(salario) INTO p_media_sal FROM atleta
...
-- exemplo do uso de parametros modo IN OUT
-- criando um bloco que chama a procedure PR_TROCA_VAL
DECLARE
    v_val1 NUMBER := 5;
    v_val2 NUMBER := 10;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Valor de v_val1: ' || v_val1 || ', Valor de v_val2: ' || v_val2);
...
CREATE OR REPLACE PROCEDURE PR_ALTERA_DADOS (
    ...
    p_cpf IN atleta.cpf%TYPE,
    p_dtnasc IN atleta.datanasc%TYPE) IS
    v_aux NUMBER;
BEGIN
    SELECT COUNT(*) INTO v_aux FROM atleta WHERE id = p_id;

    IF v_aux > 0 THEN
        UPDATE atleta SET nome = p_nome, salario = p_sal, datanasc = p_dtnasc, cpf = p_cpf
        WHERE id = p_id;
        COMMIT;
...
CALL PR_ALTERA_DADOS (18, 'Teste', p_cpf => '444', p_dtnasc => to_date('10/02/1995','dd/mm/yyyy'));


CREATE FUNCTION AREA_CIRSULO (
    p_raio IN NUMBER
) RETURN NUMBER AS
...
SELECT AREA_CIRSULO (2) FROM dual;

SELECT AREA_CIRSULO (p_raio => 4) FROM dual;

DROP FUNCTION AREA_CIRSULO;

CREATE FUNCTION FUN_GET_MEDIA_SAL (
...
BEGIN
    ...
    RETURN v_media;
END FUN_GET_MEDIA_SAL;

-- chamando para um clube especifico
SELECT FUN_GET_MEDIA_SAL(8) as media_sal_clube FROM dual;

-- chamando para todos os clubes
SELECT id, nome, round(FUN_GET_MEDIA_SAL(id),2) media_sal_atletas FROM clube;
...
-- 1. Faça uma função chamada fu_calcula_reajuste que
-- receba por parâmetro o ID de um atleta e o percentual
-- de reajuste do salário. A função deve aplicar o
-- percentual de reajuste ao salário e retornar o novo
-- salário calculado do atleta. Depois, atualize o salário de
-- algum atleta utilizando a função criada.
...
-- informado. Utilize cursor para resolver o exercício e treinar sua
-- utilização. Depois, execute a procedure reajustando os salários
-- dos atletas de algum clube.



-- 3. Faça uma procedure chamada pr_calcula_reajuste2
-- que transforme a função feita no exercício 1 em uma
-- procedure. Acrescente um parâmetro de saída que
-- recebe o novo salário do atleta. Crie um bloco PL/SQL
-- para chamar a procedure.


-- 4. Faça uma procedure chamada PR_VAGAS_CLUBE que receba por
-- parâmetro 3 IDs de clubes. A procedure deve retornar o número de vagas
-- para atletas paraolímpicos disponíveis em cada clube, conforme a regra
-- abaixo:
...
-- A procedure deve permitir que o usuário a execute informando um, dois ou
-- três IDs de clubes, ou seja, o segundo e terceiro parâmetros devem ser
-- opcionais. Execute a procedure várias vezes, informando 1, 2 e 3 IDs de
-- clubes.
...
-- 5. Modifique o exercício 4, criando agora uma função auxiliar chamada
-- FU_VAGAS_PARAOL, que recebe por parâmetro o ID de um clube e a
-- quantidade de atletas paraolímpicos desse clube, retornando o
-- número de vagas paraolímpicas disponíveis para o clube, seguindo a
-- mesma regra do exercício anterior.
-- Modifique a procedure criada anteriormente, fazendo agora uso da
-- função FU_VAGAS_PARAOL dentro da procedure para auxiliar no
-- cálculo das vagas disponíveis.
-- Caso não queira substituir a procedure anterior, pode modificá-la e
-- salvá-la como PR_VAGAS_CLUBE2.
-- Execute a procedure algumas vezes, informando IDs de clubes
-- distintos, certificando-se que funciona.





-- 6. Faça uma procedure chamada PR_CLUBE_FONE_CT que recebe por
-- parâmetro o ID de um clube e escreva na tela o seu nome e o(s) seu(s)
...
-- Note que o nome do clube deve aparecer somente uma vez, ainda
-- que ele possua várias telefones nos centros de treinamentos. Os
-- telefones devem vir numerados sequencialmente, conforme o
-- exemplo.
...
-- 7. Faça uma procedure chamada PR_ATLETA_EXP que recebe por parâmetro a descrição
-- de uma modalidade esportiva e um tempo de experiência. Então, liste os nomes dos
-- atletas que praticam essa modalidade e que têm experiência maior ou igual à informada
-- e os que a praticam e têm experiência menor que a informada.
...
-- Note que o cabeçalho deve aparecer somente uma vez para quem possui mais ou igual a
-- 20 anos de experiência e uma vez apenas para quem possui até 20 anos de experiência
-- na modalidade.
...
-- 8. Faça uma procedure chamada PR_PREMIO_CAMP que
-- receba 1 parâmetro de entrada (número do campeonato) e
-- 2 parâmetros de saída (nome do campeonato e total em
-- premiação do campeonato).
-- Depois, monte um bloco PL/SQL que chame a procedure
-- criada e imprima na tela o ID do campeonato, seu nome e o
-- total em premiação dada no campeonato.





-- 9. Faça uma procedure chamada PR_ATUALIZA_CLUBE que receba os
...
-- Caso seja passado A no primeiro parâmetro, a procedure deve
-- verificar se o clube existe (por meio do ID). Caso sim, atualize suas
-- informações, caso não, insira o novo clube.
